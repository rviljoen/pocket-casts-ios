    ...
    func markAllSynced(episodeIDs ids: [String], dbQueue: FMDatabaseQueue) {
        ...
        dbQueue.inDatabase { db in
            do {
                db.beginTransaction()
                if FeatureFlag.markAllSyncedInSingleStatement.enabled {
                    try db.executeUpdate("UPDATE \(DataManager.episodeTableName) SET playingStatusModified = 0, playedUpToModified = 0, durationModified = 0, keepEpisodeModified = 0, archivedModified = 0 WHERE uuid IN (\(ids.joined(separator: ",")))", values: nil)
                } else {
