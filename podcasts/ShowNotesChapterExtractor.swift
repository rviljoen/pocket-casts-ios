...
class ShowNotesChapterExtractor {
    ...
    private static let titleExtractionPatterns = [
        ...
        #"\[(\d{1,2}:\d{2}(?::\d{2})?)\]\s+([^\n\r\[<]+?)(?=$|\n|\r|\[\d{1,2}:\d{2})"#,
        // Pattern: plain text timestamp followed by space and text - handles both MM:SS and H:MM:SS formats
        #"(\d{1,2}:\d{2}(?::\d{2})?)\s+([A-Za-z][^0-9]*?)(?=\s+\d{1,2}:\d{2}|$)"#,
        // Pattern: timestamp followed by dash/colon and text - handles both MM:SS and H:MM:SS formats
        #"(\d{1,2}:\d{2}(?::\d{2})?)\s*[-–—:]\s*([A-Za-z][^0-9]*?)(?=\s+\d{1,2}:\d{2}|$)"#
    ]
